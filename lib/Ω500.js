	...
	return {
		...
		preload: function (name) {
			...
			return function () {
				...
				Omega.evt.progress.map(function (p) {
				...
				if (assetsToLoad === 0 && pageLoaded) {
					...
					preloading = false;
					Omega.evt.onloads.map(function (o) {
						o();
					});
				}
...
(function(Omega){
  ...
  Omega.Class = function(){};

  // Create a new Class that inherits from this class
  Omega.Class.extend = function(prop) {
...
(function (Omega) {
	...
	var Timer = Omega.Class.extend({

		init: function (time, cb, done) {

			Omega.timers.add(this);
	...
	Omega.timer = function (time, cb, done) {
...
(function(Omega) {
	...
	Omega.utils = Omega.utils || {};
	Omega.utils.State = State;
...
(function (Omega) {
	...
	colors = {
		...
		get: function (idx) {
			return palette[Omega.math.clamp(idx % palette.length, 0, palette.length - 1)];
		},
...
(function (Omega) {
	...
	var gfx = {
		...
		loadImage: function (path, cb, flipFlags) {
			...
			var resolve = Omega.preload(path),
				image = new Image(),
				self = this,
				onload = function () {
...
(function (Omega) {
	...
	input = {
		...
		init: function (dom, icade) {
			...
			icade = icade || Omega.urlParams.icade;
...
(function (Omega) {
	...
	var Image = Omega.Class.extend({
		...
		init: function (path, flipFlags, scale) {
			...
			Omega.gfx.loadImage(path, function (img){
...
(function (Omega) {
    ...
    var Tiled = Omega.Class.extend({
        ...
        init: function (file, onload) {
            ...
            Omega.utils.ajax(file, function (xhr) {
        ...
        layer: function (name) {
            ...
            var layer = Omega.utils.getByKeyValue(this.layers, "name", name),
                tileH = this.tileH,
...
(function (Omega) {
	...
	var TrackingCamera = Omega.Camera.extend({
		...
		tick: function () {
			...
			var center = Omega.math.center(this, this.zoom),
				e = this.entity,
				xr = this.xRange,
				yr = this.yRange;
		...
		render: function (gfx, renderables) {
			...
			this._super(gfx, renderables.concat([{
				render: function (gfx, cam) {
					...
					var center = Omega.math.center(cam, cam.zoom);
	...
	Omega.TrackingCamera = TrackingCamera;
...
(function (Omega) {
	...
	var Particle = Omega.Class.extend({
...
(function (Omega) {
	...
	var Dialog = Omega.Class.extend({
		...
		tick: function (delta) {
			...
			if (this.killKey && Omega.input.pressed(this.killKey)) {
				Omega.input.release(this.killKey);
				this.done();
			}
...
(function (Omega) {
	...
	*/
	var Velocity = Omega.Trait.extend({
	...
	var Gravity = Omega.Trait.extend({
		...
		tick: function (t) {
			...
			if (this.falling) {
				t.accY += 0.25;
				t.accY = Omega.math.clamp(t.accY, 0, 20);
			} else {
				t.accY = 0;
			}
	...
	*/
	var RemoveAfter = Omega.Trait.extend({
	...
	*/
	var Sin = Omega.Trait.extend({
		...
		tick: function (t) {
			...
			this[t.target] += Math.sin(Omega.utils.now() / t.speed) * (t.amp / 10);
...
(function (Omega) {
	...
	var SpriteSheet = Omega.Class.extend({
		...
		flipImage: function (img, flags) {
			...
			var ctx = Omega.gfx.createCanvas(
...
(function (Omega) {
    ...
    var SpriteAtlas = Omega.Class.extend({
        ...
        initPList: function (plist) {
            ...
            var resolve = Omega.preload(plist);
            Omega.utils.ajax(plist, function (xhr) {
        ...
        initCSV: function (path) {
            ...
            var resolve = Omega.preload(path);
            Omega.utils.ajax(path + ".txt", function (xhr) {
                Omega.gfx.loadImage(path + ".png", function (img) {
        ...
        loadImages: function (imageData) {
            ...
            imageData.forEach(function (imgData) {
                Omega.gfx.loadImage(self.path + imgData.path, function (img) {
...
}(window.Omega));(function (Omega) {

	"use strict";

	var Anim = Omega.Class.extend({
		...
		tick: function () {
			...
			var diff = Omega.utils.now() - this.frameTime;
			this.changed = false;
			this.rewound = false;

			if (diff > this.speed) {
				this.frameTime = Omega.utils.now() + (Math.min(this.speed, diff - this.speed));
		...
		reset: function () {
			this.curFrame = 0;
			this.frameTime = Omega.utils.now();
		},
...
(function (Omega) {
	...
	var Map = Omega.Class.extend({
		...
		getBlockEdge: function(pos, vertical) {
		    ...
		    return Omega.math.snap(pos, snapTo);
		...
		*/
		imgToCells: function (img, colourMap, cb, flipFlags) {
			...
			if (typeof img === "string") {
				// Load first
				Omega.gfx.loadImage(img, function (canvas){
...
(function (Omega) {
	...
	var DebugMap = Omega.Map.extend({

		init: function (tileW, tileH, xTiles, yTiles, cells, walkable, seed) {

			var ctx = this.ctx = Omega.gfx.createCanvas(tileW * xTiles, tileH * yTiles),
				data = Omega.gfx.ctx.createImageData(tileW * xTiles,tileH * yTiles),
				pix = data.data,
			...
			oldSeed = Omega.utils.rnd.seed;
			Omega.utils.rnd.seed = this.seed;
			off = Omega.utils.rnd.rand(255);

			for (var i = 0; i < numPix; i++) {
				var row = i / data.width | 0,
					col = ((i / tileW) | 0) % data.width % xTiles,
					noise = Omega.utils.rnd.rand(100) < 30 ? (Omega.utils.rnd.rand(30)) : 0,
			...
			Omega.utils.rnd.seed = oldSeed;

			ctx.putImageData(data, 0, 0);

			this._super(
				new Omega.SpriteSheet(ctx.canvas, tileW, tileH),
				cells,
				walkable);
...
(function (Omega) {
	...
	RayCastMap = Omega.Map.extend({
		...
		setStripWidth: function (stripWidth) {
			...
			this.numRays = Math.ceil(Omega.env.w / stripWidth);
		...
		setFOV: function (fov) {
			...
			var fovRad = Omega.utils.deg2rad(fov);
			this.viewDistance = (Omega.env.w / 2) / Math.tan((fovRad / 2));
		...
		castRays: function (entities, p) {
			...
			for (i = 0; i < this.numRays; i++) {
				...
				hit = Omega.rays.cast(p.rotation + rayAngle, p.x + p.w / 2, p.y + p.h / 2, this, entities);
...
(function (Omega) {
	...
	var Game = Omega.Class.extend({
		...
		init: function (w, h) {
			...
			Omega.env.w = ctx.canvas.width;
			Omega.env.h = ctx.canvas.height;

			Omega.gfx.init(ctx);
			Omega.input.init(ctx.canvas);

			Omega.evt.onload(function () {
				self.load();
				self.run(Date.now());
			});
			...
			Omega.utils.now = function () {
				return self.now();
			};

			this.stats = Omega.utils.Stats();
		...
		setScreen: function (screen, opts) {
			...
			if (this.screenPrev) {
				...
				Omega.timer(opts.time || 20, function (ratio) {
